...
pub fn insert(conn: &Connection, patch: &EntityPatch) -> Result<i64, StorageError> {
    ...
    conn.execute(
        "INSERT INTO entities(type, name, attrs) VALUES (?1, ?2, ?3)",
        params![type_, name, attrs],
    )?;
...
pub fn update(conn: &Connection, id: i64, patch: &EntityPatch) -> Result<(), StorageError> {
    ...
    let attrs = if patch.attrs.is_empty() {
        None
    } else {
        Some(Value::Object(patch.attrs.clone()).to_string())
    };
    conn.execute(
...
pub fn get(conn: &Connection, id: i64) -> Result<Option<EntitySummary>, StorageError> {
    let raw = conn
        .query_row(
            "SELECT id, type, name, attrs FROM entities WHERE id = ?1",
            params![id],
            raw_row,
        )
        .optional()?;
...
pub fn search_like(
    ...
    let raws: Vec<RawEntity> = stmt
        .query_map(params![like_pattern(needle), limit as i64], raw_row)?
        .collect::<Result<_, _>>()?;
...
mod tests {
    ...
    fn attrs_round_trip_and_search() {
        ...
        db.with_conn::<_, StorageError>(|c| {
            let id = insert(
                c,
                &serde_json::from_value::<EntityPatch>(
                    json!({"type": "product", "name": "gaia-library", "attrs": {"lang": "rust"}}),
                )
                .unwrap(),
            )?;
            ...
            update(
                ...
                id,
                &serde_json::from_value::<EntityPatch>(
                    json!({"attrs": {"lang": "rust", "db": "sqlite"}}),
                )
                .unwrap(),
            )?;
            ...
            assert!(matches!(
                insert(
                    c,
                    &serde_json::from_value::<EntityPatch>(json!({"name": "x"})).unwrap()
                ),
