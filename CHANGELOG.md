...
- `narumi` の起動コマンドは設定ファイルからのみ読み、ツール引数では指定できない。子プロセスの stdout は MCP 専用、stderr は既定で破棄し、タイムアウトで停止してプロセスグループごと終了させる。narumi へは参照行の scope 1 つだけを渡す。解決器ごとに同時実行数を制限する。
